...
const WeatherCard = ({ weather, className = "" }: WeatherCardProps) => {
  ...
  const getBackgroundClass = (main: string) => {
    switch (main.toLowerCase()) {
      case "clear":
        return "from-weather-clear/40 to-blue-50/20";
      case "clouds":
        return "from-weather-clouds/40 to-gray-100/20";
      case "rain":
        return "from-weather-rain/40 to-blue-100/20";
      case "snow":
        return "from-weather-snow/40 to-blue-50/20";
      case "thunderstorm":
        return "from-weather-thunderstorm/40 to-indigo-100/20";
      case "drizzle":
        return "from-weather-drizzle/40 to-blue-100/20";
      case "mist":
      case "fog":
      case "haze":
        return "from-weather-mist/40 to-gray-100/20";
      default:
        return "from-blue-100/40 to-blue-50/20";
    }
  };
  ...
  return (
    <div 
      className={`p-6 rounded-2xl glass bg-gradient-to-br ${getBackgroundClass(weather.main)} shadow-xl transition-all duration-500 animate-fade-up text-gray-800 ${className}`}
    >
      <div className="flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="flex-1 flex flex-col items-center md:items-start">
          ...
          <p className="text-sm text-gray-600 mb-2">Feels like: {weather.feels_like}°C</p>
          <h2 className="text-2xl md:text-3xl font-medium mt-1">{weather.city}, {weather.country}</h2>
          <p className="text-gray-600">{formatDate(weather.date)}</p>
        </div>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
        <div className="flex items-center gap-3 bg-white/30 p-4 rounded-xl">
